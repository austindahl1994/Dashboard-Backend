...
export const checkPoints = (allSheets) => {
  try {
    ...
    let teamGP = [0, 0, 0];
    keys.forEach((discordUsername) => {
      // Player earned points through recurring bounties
    ...
    allSheets.forEach((sheet, tabIndex) => {
      ...
      sheet.forEach((rowObj) => {
        ...
        if (status === "completed" || status === "manually claimed") {
          ...
          if (!player) {
            console.log(
              `Player from sheets with discord: ${rowObj.Discord} was not found in cached players`
            );
          } else {
...
const displayResults = (data) => {
  ...
  const teamTable = [
    ...
    {
      Team: "Draynor Cabbage Mafia",
      Points: teamPoints[1],
      GP: formatBounty(teamGP[1]),
    },
    {
      Team: "Futt Buckers",
      Points: teamPoints[2],
      GP: formatBounty(teamGP[2]),
    },
  ];
